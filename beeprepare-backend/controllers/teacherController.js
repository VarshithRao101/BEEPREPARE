...
const upload = multer({
  ...
  limits: { fileSize: 8 * 1024 * 1024 }, // 8MB hard cap (per-type check in controller)
...
const getNotes = async (req, res) => {
  try {
    ...
    // 2. Fetch all notes for this bank from Firestore
    const notesSnapshot = await db.collection('notes')
      .where('bankId', '==', bankId)
      .get();
    ...
    const chaptersWithNotes = syllabusChapters.map(chap => {
      ...
      console.log(`Syncing notes for chapter: ${chap.chapterName} (ID: ${cId})`);
      return {
        chapterId: cId,
        ...
        notes: notes.filter(n => n.chapterId === cId).map(n => ({
          ...
          fileUrl: n.fileUrl,
          noteType: n.noteType,
          uploadedAt: n.createdAt?.toDate ? n.createdAt.toDate() : n.createdAt
        }))
      };
    });
...
const uploadNote = async (req, res) => {
  try {
    ...
    const teacherId = req.user.googleUid;
    console.log('Params:', { bankId, chapterId, chapterName, noteType, teacherId });

    if (!bankId || !chapterId || !chapterName || !noteType) {
      console.error('MISSING_FIELDS:', req.body);
      return error(res, 'bankId, chapterId, chapterName, and noteType are required', 'MISSING_FIELDS', 400);
    }

    if (!['complete', 'short'].includes(noteType)) {
      return error(res, "noteType must be 'complete' or 'short'", 'INVALID_NOTE_TYPE', 400);
    }

    if (!req.file) {
      return error(res, 'File is required', 'NO_FILE', 400);
    }

    // Enforce size limits
    const maxSize = noteType === 'complete' ? 7 * 1024 * 1024 : 4 * 1024 * 1024;
    if (req.file.size > maxSize) {
      const maxMB = noteType === 'complete' ? 7 : 4;
      return error(res, `${noteType} notes cannot exceed ${maxMB}MB`, 'FILE_TOO_LARGE', 413);
    }
    ...
    // Check for existing note of same type in same chapter in Firestore
    const existingSnapshot = await db.collection('notes')
      .where('bankId', '==', bankId)
    ...
    if (!existingSnapshot.empty) {
      ...
      if (existingData.fileUrl) await deleteFromStorage(existingData.fileUrl);
    ...
    const fileName = `${noteType}_${chapterId}_${Date.now()}.${ext}`;
    const storagePath = `notes/${teacherId}/${bankId}/${chapterId}/${noteType}/${fileName}`;

    // Upload to Firebase Storage
    console.log('Uploading to bucket path:', storagePath);
    
    // FIX: Use the bucket object from config/firebase.js directly
    ...
    // Dynamically construct URL from bucket name to avoid hardcoding issues
    ...
    await logActivity(
      ...
      'note_uploaded',
      'Note Uploaded',
      `Uploaded ${noteType} note for ${chapterName}`,
      '#9C27B0'
    );

    return success(res, 'Note uploaded successfully', {
      noteId: docRef.id,
      fileUrl,
      fileName,
      fileSize: req.file.size,
    ...
    console.error('CRITICAL uploadNote error:', err);
    return error(res, `Failed to upload note: ${err.message}`, 'SERVER_ERROR', 500);
  }
};
...
const deleteNote = async (req, res) => {
  try {
    ...
    if (!noteDoc.exists) return error(res, 'Note not found', 'NOT_FOUND', 404);
    const noteData = noteDoc.data();
    
    if (noteData.teacherId !== teacherId) return error(res, 'Access denied', 'FORBIDDEN', 403);

    if (noteData.fileUrl) await deleteFromStorage(noteData.fileUrl);

    await noteRef.delete();
    await Bank.updateOne({ _id: noteData.bankId }, { $inc: { notesCount: -1 } });

    return success(res, 'Note deleted successfully', null);
  } catch (err) {
    console.error('deleteNote error:', err);
    return error(res, 'Failed to delete note', 'SERVER_ERROR', 500);
  }
};
